...
void main(void)
{
	...
	lcz_hw_key_encrypt_data((const uint8_t *)iv, LCZ_HW_KEY_IV_LEN, (const uint8_t *)plain_text,
				strlen(plain_text), encrypted_text, sizeof(encrypted_text),
				&encrypted_data_size);

	LOG_HEXDUMP_INF(encrypted_text, encrypted_data_size, "Encrypted data:");

	lcz_hw_key_decrypt_data((const uint8_t *)iv, LCZ_HW_KEY_IV_LEN,
				(const uint8_t *)encrypted_text, encrypted_data_size,
				decrypted_text, sizeof(decrypted_text), &decrypted_data_size);
